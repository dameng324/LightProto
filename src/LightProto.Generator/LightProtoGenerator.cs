...
public class LightProtoGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(
            protoContracts.Collect().Combine(context.CompilationProvider),
            (spc, pair) =>
            {
                var (types, compilation) = pair;
                if (types.Length == 0)
                    return;
                var moduleInitializerType = compilation.GetTypeByMetadataName(
                    "System.Runtime.CompilerServices.ModuleInitializerAttribute"
                );
                if (moduleInitializerType is null)
                    return;

                var source = GenerateRegistry(types);
                spc.AddSource("LightProtoRegistry.g.cs", source);
            }
        );
    }

    private static string GenerateRegistry(ImmutableArray<ISymbol?> types)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("internal static class ProtoRegistry");
        sb.AppendLine("{");
        ...
        foreach (var type in types)
        {
            if (type is null)
                continue;
            if (type.ContainingAssembly.Name == "LightProto")
                continue;
            var fqName = type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            if (
                type is INamedTypeSymbol namedTypeSymbol
                && namedTypeSymbol.TypeKind is TypeKind.Interface
            )
                sb.AppendLine(
                    $" global::LightProto.Serializer.RegisterParser({fqName}ProtoParser.ProtoReader,{fqName}ProtoParser.ProtoWriter);"
                );
            else
                sb.AppendLine(
                    $" global::LightProto.Serializer.RegisterParser({fqName}.ProtoReader,{fqName}.ProtoWriter);"
                );
        }
        sb.AppendLine(" }");
        sb.AppendLine("}");
    ...
    )
    {
        ...
        if (memberType is INamedTypeSymbol namedType)
        {
            ...
            if (typeArguments.Length == 0)
            {
                ...
                {
                    return $"LightProto.Parser.EnumProto{readerOrWriter}<{namedType}>.Default";
                }
