...
{
    public ConcurrentQueueProtoWriter(IProtoWriter<T> itemWriter, uint tag, int itemFixedSize,bool isPacked)
...
{
    public ConcurrentQueueProtoReader(IProtoReader<T> itemReader, uint tag, int itemFixedSize,bool isPacked)
        : base(itemReader, tag,static (size) => new ConcurrentQueue<T>(), static (collection,item) =>
        {
