...
internal sealed class LightProtoCSharpGenerator
{
    ...
    private static readonly HashSet<string> CSharpReservedIdentifiers = new(StringComparer.Ordinal)
    {
        "abstract",
        "as",
        "base",
    ...
    private string ResolveTypeCsName(string protoTypeName)
    {
        ...
        for (int i = 0; i < firstTypeIndex; i++)
        {
            if (fullNameBuilder.Length > 0)
                fullNameBuilder.Append('.');
            fullNameBuilder.Append(sourceSegments[i]);
        }
        ...
        for (int i = firstTypeIndex; i < sourceSegments.Length; i++)
        {
            if (fullNameBuilder.Length > 0)
                fullNameBuilder.Append('.');
            fullNameBuilder.Append(sourceSegments[i]);
            converted.Add(ConvertIdentifier(sourceSegments[i], fullNameBuilder.ToString()));
        }
    ...
    private string ConvertIdentifier(string protoName, string protoFullName)
    {
        ...
        return SanitizeCSharpIdentifier(converted);
    }

    private static string SanitizeCSharpIdentifier(string identifier)
    {
        ...
        {
            sb.Append(IsValidIdentifierPart(ch) ? ch : '_');
        }

        if (sb.Length == 0)
            sb.Append("Generated");

        if (!IsValidIdentifierStart(sb[0]))
            sb.Insert(0, '_');

        var sanitized = sb.ToString();
        return CSharpReservedIdentifiers.Contains(sanitized) ? "@" + sanitized : sanitized;
    }

    private static bool IsValidIdentifierStart(char ch) => ch == '_' || char.IsLetter(ch);

    private static bool IsValidIdentifierPart(char ch) => ch == '_' || char.IsLetterOrDigit(ch);
