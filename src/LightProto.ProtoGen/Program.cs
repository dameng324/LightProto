...
internal class Program
{
    public static int Main(string[] args)
    {
                ...
                if (protoPatterns.Length == 0)
                {
                    if (!Console.IsInputRedirected)
                    {
                        Console.Error.WriteLine(
                            "error: No .proto files specified and no input on stdin. "
                                + "Provide --proto <file> or pipe proto schema to stdin."
                        );
                        return 1;
                    }
        ...
        static string GenerateFromProtoSchema(
            string protoSchema,
            string fileName,
            LightProtoCSharpGenerator generator,
