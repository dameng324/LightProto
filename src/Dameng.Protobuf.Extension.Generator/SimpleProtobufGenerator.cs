...
public class SimpleProtobufGenerator : ISourceGenerator
{
    ...
    {
        return IsArrayType(type) || IsListType(type) || IsSetType(type);
    }
    ...
    static ITypeSymbol GetElementType(ITypeSymbol collectionType)
    {
        ...
        if ((IsListType(collectionType) || IsSetType(collectionType)) && collectionType is INamedTypeSymbol namedType)
        {
            return namedType.TypeArguments[0];
        }
        throw new ArgumentException("Type is not an array, list, or set type", nameof(collectionType));
    }
    ...
    static string GetPbTypeString(ITypeSymbol genericType, DataFormat memberDataFormat)
    {
        return genericType.SpecialType switch
        {
            ...
            _ when IsGuidType(genericType) => "String",
            _ when IsTimeSpanType(genericType) => "Message",
            _ when IsDateOnlyType(genericType) => "String",
            _ when IsTimeOnlyType(genericType) => "String",
