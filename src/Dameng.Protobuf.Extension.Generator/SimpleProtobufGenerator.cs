...
public class SimpleProtobufGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        foreach (var syntaxTree in context.Compilation.SyntaxTrees)
        {
            ...
            foreach (var node in syntaxTree.GetRoot().DescendantNodesAndSelf())
            {
                ...
                var fileName = $"{targetType.Name}.g.cs";
            ...
            $$"""
              {
                      ...
                      GetProtoMembers(targetType).Select(member => {
                          ...
                          if (member.Type.ToDisplayString().StartsWith("Google.Protobuf.Collections.MapField"))
                          {
                              ...
                              var keyDataFormatString = member.AttributeData
                                  .FirstOrDefault(o => o.AttributeClass?.ToDisplayString() == "Dameng.Protobuf.Extension.MapKeyDataFormatAttribute")
                                  ?.NamedArguments
                                  .FirstOrDefault(pair => pair.Key == "DataFormat")
                              ...
                              var valueDataFormatString = member.AttributeData
                                  .FirstOrDefault(o => o.AttributeClass?.ToDisplayString() == "Dameng.Protobuf.Extension.MapValueDataFormatAttribute")
                                  ?.NamedArguments
                                  .FirstOrDefault(pair => pair.Key == "DataFormat")
                                  .Value.Value?.ToString();
                  ...
                  {{string.Join(Environment.NewLine,
                      GetProtoMembers(targetType).Select(member => {
                          ...
                          if (member.Type.SpecialType == SpecialType.System_String)
                              return $"            if({member.Name}.Length !=0) {{ output.WriteRawTag({member.RawTag}); output.Write{pbTypeString}({member.Name});}}";
                          if (member.Type.TypeKind == TypeKind.Enum)
                              return $"            if({member.Name} != default) {{ output.WriteRawTag({member.RawTag}); output.Write{pbTypeString}((int){member.Name});}}";
                          if (member.Type.IsValueType)
                              return $"            if({member.Name} != default) {{ output.WriteRawTag({member.RawTag}); output.Write{pbTypeString}({member.Name});}}";
                          if (member.Type.ToDisplayString() == "Google.Protobuf.ByteString")
                              return $"            if({member.Name}.Length !=0) {{  output.WriteRawTag({member.RawTag}); output.Write{pbTypeString}({member.Name});}}";

                          return $"            if ({member.Name} != null) {{  output.WriteRawTag({member.RawTag}); output.Write{pbTypeString}({member.Name});}}";
                      }))
                  }}
                  }
                  ...
                  {{string.Join(Environment.NewLine,
                      GetProtoMembers(targetType).Select(member => {
                          var pbTypeString = GetPbTypeString(member.Type, member.DataFormat);
                          var lengthSize= member.RawTag / 127 + 1;
                          if (member.Type.SpecialType == SpecialType.System_String)
    ...
    static string GetPbTypeString(ITypeSymbol genericType, DataFormat memberDataFormat)
    {
        return genericType.SpecialType switch
        {
            ...
            SpecialType.System_Int32 => memberDataFormat == DataFormat.FixedSize ? "SFixed32" : memberDataFormat == DataFormat.ZigZag ? "SInt32" : "Int32",
            SpecialType.System_UInt32 => memberDataFormat == DataFormat.FixedSize ? "Fixed32" : "UInt32",
            SpecialType.System_Int64 => memberDataFormat == DataFormat.FixedSize ? "SFixed64" : memberDataFormat == DataFormat.ZigZag ? "SInt64" : "Int64",
            SpecialType.System_UInt64 => memberDataFormat == DataFormat.FixedSize ? "Fixed64" : "UInt64",
    ...
    private List<ProtoMember> GetProtoMembers(INamedTypeSymbol targetType)
    {
        ...
        foreach (var member in targetType.GetMembers())
        {
            ...
            members.Add(new ProtoMember
            {
                ...
                DataFormat =
                    Enum.TryParse<DataFormat>(
                        protoMemberAttr.NamedArguments.FirstOrDefault(kv => kv.Key == "DataFormat").Value.Value
    ...
    private class ProtoMember
    {
        ...
        public static PbWireType GetPbWireType(ITypeSymbol Type,DataFormat DataFormat)
        {
            switch (Type.SpecialType)
            {
                ...
                case SpecialType.None when Type.TypeKind == TypeKind.Class || Type.TypeKind == TypeKind.Interface ||
                                           Type.TypeKind == TypeKind.Array:
                    return PbWireType.LengthDelimited;
                default:
                    if (Type.ToDisplayString().StartsWith("Google.Protobuf.Collections.RepeatedField"))
                    {
